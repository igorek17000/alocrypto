...
<body>
    <div>
        <?php
        $is_verify = Database::getUserDetails()->is_verify;
        if ($is_verify == false) {
        ?>
            <div class="alert alert-info text-center" style="margin-top: 100px;" role="alert">
                you needed to verify your email address clike <a href="" id="vrv">here</a> to verify
            </div>
        <?php } ?>
    ...
    <script>
        $(document).ready(function() {
            $("#vrv").click(function(e) {
                e.preventDefault();
                swal("request is proccessing");
                $.ajax({
                    type: "POST",
                    url: "handler",
                    data: {
                    ...
                    success: function(response) {
                        if (response === "true") {
                            swal("Verification link as been sent to you you");
                        } else {
                            swal({
                                title: "Error occur!",
                                text: response,
